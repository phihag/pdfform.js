...
// exclude these imports from production builds
...
function pdfform(minipdf_lib) {
  // exclude these imports from production builds
  if (process.env.NODE_ENV !== "production" && minipdf_lib === "pdf.js") {
      ...
      if (n._pdfform_spec) {
        ...
        if (ft_name == "Tx") {
          ...
          n.map.AS =
            n.map.V =
            n.map.DV =
              value ? new minipdf_lib.Name("Yes") : new minipdf_lib.Name("Off");
        } else if (ft_name == "Ch") {
          n.map.V = "" + value;
        } else if (ft_name == "Sig") {
