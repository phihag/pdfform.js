var minipdf = (function () {
  ...
  PDFDocument.prototype.fetch = function (ref, recursive) {
    ...
    if (obj.num !== ref.num) {
      throw new Error(
        "Expected to read object with ID " + ref.num + ", but found " + obj.num
      );
    }
    if (obj.gen !== ref.gen) {
      throw new Error(
        "Expected to read object with gen " + ref.gen + ", but found " + obj.gen
      );
    }
    return obj.obj;
  };
  ...
  PDFReader.prototype = {
    ...
    read_uint: function (len) {
      ...
      while (len > 0) {
        assert(
          ...
          "reading uint at position " + this.pos + " of " + this.buf.length
        );
    ...
    parse_name: function () {
      var start_pos = this.pos;
      var DELIM_CHARS = [
        0, 9, 13, 10, 32, 40, 41, 60, 62, 91, 93, 123, 125, 47, 37,
      ];
      while (this.pos < this.buf.length) {
      ...
      if (map.Filter) {
        ...
        for (var i = 0; i < filters.length; i++) {
          ...
          switch (filters[i].name) {
            ...
            default:
              throw new Error(
                "Unsupported filter: " + JSON.stringify(filters[i].name)
              );
          }
        }
      }
